...
- **Journal d'audit** (admin) — table `audit_event` (migration **V20**) : connexions (réussies/refusées, IP) et verrouillages, création/clôture de mission, réaffectation, gestion des comptes, **export/import de la base** ; écran filtrable et paginé (`/journal`).
- **Export / import de la base** (admin) — écran `/donnees` : export d'une sauvegarde complète (`pg_dump`, format custom, téléchargeable) et import/restauration **atomique** (`pg_restore --single-transaction`, remplace les données) ; pré-requis : outils `pg_dump`/`pg_restore` accessibles (variable `PG_BIN` sous Windows) ; opérations tracées.
