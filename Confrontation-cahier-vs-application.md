...
- **Agents en charge & ordre de mission** (§9.16) — la **liste des missions** affiche les informaticiens en charge (chef de mission en tête) ; un **ordre de mission PDF** facultatif (un par mission) est **joignable** depuis la liste (ADMIN / Chef), **téléchargeable** par tout utilisateur connecté, **remplaçable** et **supprimable**. Stockage en base (table `ordre_mission`, BYTEA — migration **V17**) ; seul le **PDF** est accepté.
