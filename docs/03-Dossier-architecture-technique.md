...
| Migrations | **Flyway** (V1→V8) | Schéma versionné |
| Sécurité | **Spring Security** | Authentification, rôles, CSRF |
...
- **`web/`** — contrôleurs MVC (HomeController, ConsultationController, AgentController, PosteController, MissionController, ReferentielController, UtilisateurController…), rendu Thymeleaf.
- **`*service` / `mission`, `agent`, `affectation`, `importation`, `consultation`** — logique métier (création, validation, historisation, consolidation, restitutions).
- **`repository/`** — interfaces Spring Data JPA (requêtes dérivées + `@Query`).
- **`domain/`** — entités JPA (modèle).
- **`consultation/dto/`, `importation/dto/`** — objets de transfert/vues pour l'affichage et l'import.

### Composants transverses

- **`GlobalModelAdvice`** (`@ControllerAdvice`) : expose à toutes les vues l'utilisateur connecté (`currentUser`), son rôle (`currentRole`) et la page courante (`currentPath`).
- **`SecurityConfig`** : chaîne de filtres (form login, règles par URL/rôle, BCrypt, déconnexion, **CSRF actif**).
- **`fragments/layout.html`** : menu latéral commun ; **`static/css/app.css`** : thème.
...
- **Trois rôles** (ADMIN, CHEF_MISSION, AGENT) ; règles d'accès par URL et par méthode HTTP.
...
- **Production** : profil dédié (HTTPS via keystore PKCS12, cookies de session sécurisés), compte `admin/admin` initial à changer, accès réseau restreint à l'intranet.
...
- **Exécution** : `java -jar` (Tomcat embarqué) ; profil `prod` pour la production.
- **Base** : PostgreSQL ; schéma appliqué automatiquement par **Flyway** au démarrage.
- **Configuration** : variables d'environnement (`DB_URL`, `DB_USER`, `DB_PASSWORD`…) ou `application-local.properties` (non versionné).
- **Sauvegardes** : scripts `scripts/sauvegarde-postgres.sh` / `restauration-postgres.sh`.

Voir [06 — Guide d'installation et d'exploitation](06-Guide-installation-exploitation.md).
