...
1. **Créer la mission** (admin/chef) : TPR, objet, période, membres (informaticiens), chef de mission. Le **chef de poste est facultatif à la création** (laissé vide s'il est inconnu — il sera alors **obligatoirement** renseigné dans le canevas). Un même agent peut être membre de **missions aux périodes qui se chevauchent** (aucun blocage). L'application génère le **N° de mission**.
2. **Télécharger le canevas pré-rempli** et le distribuer aux agents. Le ZIP est nommé par **code poste + période** (ex. `Canevas-DKR-2026-06-01_2026-06-05.zip`) et chaque fichier reprend ce nom suivi de l'agent. Pour plusieurs missions à la fois : depuis la **liste des missions**, cocher les missions voulues puis **« Télécharger la sélection »** (un seul ZIP). *(Admin / Chef de mission)*
3. **Saisir hors-ligne** dans Excel (un fichier par agent possible). Respecter les champs obligatoires et le format des **MAC** (`AA:BB:CC:DD:EE:FF`). Laisser le **n° d'inventaire vide** pour un matériel nouveau. Le **chef de poste** (désormais **obligatoire** dans le canevas — sans lui le chargement est refusé) et les **dates de mission** renseignés dans le canevas sont **reportés à la mission** à l'import. Logiciels relevés sur les ordinateurs : Aster, Antivirus, SicCDD, CIC, Sysbudget, **AD** (colonnes Oui/Non). *Voir le **[Guide de remplissage du canevas](../Guide-remplissage-canevas.md)** pour le détail des onglets et des contrôles.*
...
> **Liste des missions** : **filtrable** par **poste**, **région**, **agent de mission** et **état**. Chaque ligne indique les **agents en charge** (chef de mission en tête, mention « chef », puis les membres). On peut y **joindre un ou plusieurs ordres de mission** au format **PDF** (bouton **Joindre**, *admin / chef de mission*) ; chaque document se **télécharge** d'un clic (ouverture dans le navigateur) par tout utilisateur connecté et peut être **supprimé** individuellement.
